hoistPattern:
  - '*'
hoistedDependencies:
  /@gar/promisify/1.1.3:
    '@gar/promisify': public
  /@heroku-cli/color/1.1.14:
    '@heroku-cli/color': public
  /@heroku-cli/command/8.5.0:
    '@heroku-cli/command': public
  /@heroku-cli/heroku-exec-util/0.7.6:
    '@heroku-cli/heroku-exec-util': public
  /@heroku-cli/notifications/1.2.3:
    '@heroku-cli/notifications': public
  /@heroku-cli/plugin-addons-v5/7.67.0:
    '@heroku-cli/plugin-addons-v5': public
  /@heroku-cli/plugin-addons/1.2.31:
    '@heroku-cli/plugin-addons': public
  /@heroku-cli/plugin-apps-v5/7.67.0:
    '@heroku-cli/plugin-apps-v5': public
  /@heroku-cli/plugin-apps/7.66.3:
    '@heroku-cli/plugin-apps': public
  /@heroku-cli/plugin-auth/7.66.3:
    '@heroku-cli/plugin-auth': public
  /@heroku-cli/plugin-autocomplete/7.66.3:
    '@heroku-cli/plugin-autocomplete': public
  /@heroku-cli/plugin-buildpacks/7.66.3:
    '@heroku-cli/plugin-buildpacks': public
  /@heroku-cli/plugin-certs-v5/7.66.3:
    '@heroku-cli/plugin-certs-v5': public
  /@heroku-cli/plugin-certs/7.66.3:
    '@heroku-cli/plugin-certs': public
  /@heroku-cli/plugin-ci-v5/7.66.3:
    '@heroku-cli/plugin-ci-v5': public
  /@heroku-cli/plugin-ci/7.66.3:
    '@heroku-cli/plugin-ci': public
  /@heroku-cli/plugin-config/7.66.3:
    '@heroku-cli/plugin-config': public
  /@heroku-cli/plugin-container-registry-v5/7.66.3:
    '@heroku-cli/plugin-container-registry-v5': public
  /@heroku-cli/plugin-git/7.66.3:
    '@heroku-cli/plugin-git': public
  /@heroku-cli/plugin-local/7.66.3_debug@4.1.1:
    '@heroku-cli/plugin-local': public
  /@heroku-cli/plugin-oauth-v5/7.66.3:
    '@heroku-cli/plugin-oauth-v5': public
  /@heroku-cli/plugin-orgs-v5/7.66.3:
    '@heroku-cli/plugin-orgs-v5': public
  /@heroku-cli/plugin-pg-v5/7.66.3:
    '@heroku-cli/plugin-pg-v5': public
  /@heroku-cli/plugin-pipelines/7.66.3:
    '@heroku-cli/plugin-pipelines': public
  /@heroku-cli/plugin-ps-exec/2.4.0:
    '@heroku-cli/plugin-ps-exec': public
  /@heroku-cli/plugin-ps/7.66.3:
    '@heroku-cli/plugin-ps': public
  /@heroku-cli/plugin-redis-v5/7.66.3:
    '@heroku-cli/plugin-redis-v5': public
  /@heroku-cli/plugin-run-v5/7.67.1:
    '@heroku-cli/plugin-run-v5': public
  /@heroku-cli/plugin-run/7.67.1:
    '@heroku-cli/plugin-run': public
  /@heroku-cli/plugin-spaces/7.66.3:
    '@heroku-cli/plugin-spaces': public
  /@heroku-cli/plugin-status/7.66.3:
    '@heroku-cli/plugin-status': public
  /@heroku-cli/plugin-webhooks/7.66.3:
    '@heroku-cli/plugin-webhooks': public
  /@heroku-cli/schema/1.0.25:
    '@heroku-cli/schema': public
  /@heroku/buildpack-registry/1.0.1:
    '@heroku/buildpack-registry': public
  /@heroku/eventsource/1.0.7:
    '@heroku/eventsource': public
  /@heroku/socksv5/0.0.9:
    '@heroku/socksv5': public
  /@mapbox/node-pre-gyp/1.0.10:
    '@mapbox/node-pre-gyp': public
  /@nodelib/fs.scandir/2.1.5:
    '@nodelib/fs.scandir': public
  /@nodelib/fs.stat/2.0.5:
    '@nodelib/fs.stat': public
  /@nodelib/fs.walk/1.2.8:
    '@nodelib/fs.walk': public
  /@npmcli/fs/1.1.1:
    '@npmcli/fs': public
  /@npmcli/move-file/1.1.2:
    '@npmcli/move-file': public
  /@oclif/color/0.0.0:
    '@oclif/color': public
  /@oclif/command/1.8.16:
    '@oclif/command': public
  /@oclif/config/1.13.2:
    '@oclif/config': public
  /@oclif/errors/1.2.2:
    '@oclif/errors': public
  /@oclif/help/1.0.4:
    '@oclif/help': public
  /@oclif/linewrap/1.0.0:
    '@oclif/linewrap': public
  /@oclif/parser/3.8.9:
    '@oclif/parser': public
  /@oclif/plugin-commands/1.3.0:
    '@oclif/plugin-commands': public
  /@oclif/plugin-help/2.2.0:
    '@oclif/plugin-help': public
  /@oclif/plugin-legacy/1.2.0:
    '@oclif/plugin-legacy': public
  /@oclif/plugin-not-found/1.2.2:
    '@oclif/plugin-not-found': public
  /@oclif/plugin-plugins/1.10.1:
    '@oclif/plugin-plugins': public
  /@oclif/plugin-update/1.5.0:
    '@oclif/plugin-update': public
  /@oclif/plugin-warn-if-update-available/1.7.0:
    '@oclif/plugin-warn-if-update-available': public
  /@oclif/plugin-which/1.0.3:
    '@oclif/plugin-which': public
  /@oclif/screen/1.0.4:
    '@oclif/screen': public
  /@sindresorhus/is/0.7.0:
    '@sindresorhus/is': public
  /@szmarczak/http-timer/1.1.2:
    '@szmarczak/http-timer': public
  /@tootallnate/once/1.1.2:
    '@tootallnate/once': public
  /@types/semver/7.3.13:
    '@types/semver': public
  /abbrev/1.1.1:
    abbrev: public
  /accepts/1.3.8:
    accepts: public
  /agent-base/6.0.2:
    agent-base: public
  /agentkeepalive/4.2.1:
    agentkeepalive: public
  /aggregate-error/3.1.0:
    aggregate-error: public
  /ansi-escapes/3.2.0:
    ansi-escapes: public
  /ansi-regex/4.1.1:
    ansi-regex: public
  /ansi-styles/3.2.1:
    ansi-styles: public
  /ansicolors/0.3.2:
    ansicolors: public
  /anymatch/3.1.3:
    anymatch: public
  /app-path/3.3.0:
    app-path: public
  /aproba/2.0.0:
    aproba: public
  /are-we-there-yet/2.0.0:
    are-we-there-yet: public
  /argparse/1.0.10:
    argparse: public
  /array-flatten/1.1.1:
    array-flatten: public
  /array-union/2.1.0:
    array-union: public
  /asn1/0.2.6:
    asn1: public
  /async-file/2.0.2:
    async-file: public
  /async-limiter/1.0.1:
    async-limiter: public
  /at-least-node/1.0.0:
    at-least-node: public
  /balanced-match/1.0.2:
    balanced-match: public
  /base64-js/1.5.1:
    base64-js: public
  /bcrypt-pbkdf/1.0.2:
    bcrypt-pbkdf: public
  /binary-extensions/2.2.0:
    binary-extensions: public
  /bl/4.1.0:
    bl: public
  /bluebird/3.7.2:
    bluebird: public
  /body-parser/1.20.1:
    body-parser: public
  /brace-expansion/1.1.11:
    brace-expansion: public
  /braces/3.0.2:
    braces: public
  /buffer-equal-constant-time/1.0.1:
    buffer-equal-constant-time: public
  /buffer/5.7.1:
    buffer: public
  /buildcheck/0.0.3:
    buildcheck: public
  /byline/5.0.0:
    byline: public
  /bytes/3.1.2:
    bytes: public
  /cacache/15.3.0:
    cacache: public
  /cacheable-request/2.1.4:
    cacheable-request: public
  /call-bind/1.0.2:
    call-bind: public
  /cardinal/2.1.1:
    cardinal: public
  /chalk/2.4.2:
    chalk: public
  /chardet/0.7.0:
    chardet: public
  /chokidar/3.5.3:
    chokidar: public
  /chownr/2.0.0:
    chownr: public
  /clean-stack/1.3.0:
    clean-stack: public
  /cli-cursor/2.1.0:
    cli-cursor: public
  /cli-progress/3.11.2:
    cli-progress: public
  /cli-ux/4.9.3:
    cli-ux: public
  /cli-width/2.2.1:
    cli-width: public
  /clone-response/1.0.2:
    clone-response: public
  /co-wait/0.0.0:
    co-wait: public
  /co/4.6.0:
    co: public
  /color-convert/1.9.3:
    color-convert: public
  /color-name/1.1.3:
    color-name: public
  /color-support/1.1.3:
    color-support: public
  /commander/2.20.3:
    commander: public
  /concat-map/0.0.1:
    concat-map: public
  /console-control-strings/1.1.0:
    console-control-strings: public
  /content-disposition/0.5.4:
    content-disposition: public
  /content-type/1.0.4:
    content-type: public
  /cookie-signature/1.0.6:
    cookie-signature: public
  /cookie/0.5.0:
    cookie: public
  /core-util-is/1.0.3:
    core-util-is: public
  /cpu-features/0.0.4:
    cpu-features: public
  /cross-spawn/7.0.3:
    cross-spawn: public
  /date-fns/2.29.3:
    date-fns: public
  /debug/2.6.9:
    debug: public
  /decode-uri-component/0.2.2:
    decode-uri-component: public
  /decompress-response/3.3.0:
    decompress-response: public
  /defer-to-connect/1.1.3:
    defer-to-connect: public
  /delegates/1.0.0:
    delegates: public
  /depd/2.0.0:
    depd: public
  /destroy/1.2.0:
    destroy: public
  /detect-libc/2.0.1:
    detect-libc: public
  /dir-glob/3.0.1:
    dir-glob: public
  /duplexer3/0.1.5:
    duplexer3: public
  /ecdsa-sig-formatter/1.0.11:
    ecdsa-sig-formatter: public
  /edit-string/1.1.6:
    edit-string: public
  /ee-first/1.1.1:
    ee-first: public
  /emoji-regex/7.0.3:
    emoji-regex: public
  /encodeurl/1.0.2:
    encodeurl: public
  /encoding/0.1.13:
    encoding: public
  /end-of-stream/1.4.4:
    end-of-stream: public
  /env-paths/2.2.1:
    env-paths: public
  /err-code/2.0.3:
    err-code: public
  /error-ex/1.3.2:
    error-ex: public
  /escape-html/1.0.3:
    escape-html: public
  /escape-string-regexp/1.0.5:
    escape-string-regexp: public
  /esprima/4.0.1:
    esprima: public
  /etag/1.8.1:
    etag: public
  /eventemitter3/4.0.7:
    eventemitter3: public
  /execa/1.0.0:
    execa: public
  /external-editor/3.1.0:
    external-editor: public
  /extract-stack/1.0.0:
    extract-stack: public
  /fast-glob/3.2.12:
    fast-glob: public
  /fast-levenshtein/2.0.6:
    fast-levenshtein: public
  /fastq/1.14.0:
    fastq: public
  /figures/2.0.0:
    figures: public
  /filesize/4.2.1:
    filesize: public
  /fill-range/7.0.1:
    fill-range: public
  /finalhandler/1.2.0:
    finalhandler: public
  /follow-redirects/1.15.2_debug@4.1.1:
    follow-redirects: public
  /foreman/3.0.1_debug@4.1.1:
    foreman: public
  /forwarded/0.2.0:
    forwarded: public
  /fresh/0.5.2:
    fresh: public
  /from2/2.3.0:
    from2: public
  /fs-constants/1.0.0:
    fs-constants: public
  /fs-extra/7.0.1:
    fs-extra: public
  /fs-minipass/2.1.0:
    fs-minipass: public
  /fs.realpath/1.0.0:
    fs.realpath: public
  /fsevents/2.3.2:
    fsevents: public
  /function-bind/1.1.1:
    function-bind: public
  /gauge/3.0.2:
    gauge: public
  /get-intrinsic/1.1.3:
    get-intrinsic: public
  /get-stream/4.1.0:
    get-stream: public
  /github-url-to-object/4.0.6:
    github-url-to-object: public
  /glob-parent/5.1.2:
    glob-parent: public
  /glob/7.2.3:
    glob: public
  /globby/11.1.0:
    globby: public
  /got/8.3.2:
    got: public
  /graceful-fs/4.2.10:
    graceful-fs: public
  /growly/1.3.0:
    growly: public
  /has-ansi/2.0.0:
    has-ansi: public
  /has-flag/3.0.0:
    has-flag: public
  /has-symbol-support-x/1.4.2:
    has-symbol-support-x: public
  /has-symbols/1.0.3:
    has-symbols: public
  /has-to-string-tag-x/1.4.1:
    has-to-string-tag-x: public
  /has-unicode/2.0.1:
    has-unicode: public
  /has/1.0.3:
    has: public
  /here/0.0.2:
    here: public
  /heroku-cli-util/8.0.12:
    heroku-cli-util: public
  /heroku-client/3.1.0:
    heroku-client: public
  /http-cache-semantics/4.1.0:
    http-cache-semantics: public
  /http-call/5.3.0:
    http-call: public
  /http-errors/2.0.0:
    http-errors: public
  /http-proxy-agent/4.0.1:
    http-proxy-agent: public
  /http-proxy/1.18.1_debug@4.1.1:
    http-proxy: public
  /https-proxy-agent/5.0.1:
    https-proxy-agent: public
  /humanize-ms/1.2.1:
    humanize-ms: public
  /hyperlinker/1.0.0:
    hyperlinker: public
  /iconv-lite/0.4.24:
    iconv-lite: public
  /ieee754/1.2.1:
    ieee754: public
  /ignore-by-default/1.0.1:
    ignore-by-default: public
  /ignore/5.2.4:
    ignore: public
  /imurmurhash/0.1.4:
    imurmurhash: public
  /indent-string/3.2.0:
    indent-string: public
  /infer-owner/1.0.4:
    infer-owner: public
  /inflight/1.0.6:
    inflight: public
  /inherits/2.0.4:
    inherits: public
  /inquirer/6.5.2:
    inquirer: public
  /into-stream/3.1.0:
    into-stream: public
  /ip-address/5.9.4:
    ip-address: public
  /ip/2.0.0:
    ip: public
  /ipaddr.js/1.9.1:
    ipaddr.js: public
  /is-arrayish/0.2.1:
    is-arrayish: public
  /is-binary-path/2.1.0:
    is-binary-path: public
  /is-docker/2.2.1:
    is-docker: public
  /is-extglob/2.1.1:
    is-extglob: public
  /is-fullwidth-code-point/2.0.0:
    is-fullwidth-code-point: public
  /is-glob/4.0.3:
    is-glob: public
  /is-lambda/1.0.1:
    is-lambda: public
  /is-number/7.0.0:
    is-number: public
  /is-object/1.0.2:
    is-object: public
  /is-plain-obj/1.1.0:
    is-plain-obj: public
  /is-retry-allowed/1.2.0:
    is-retry-allowed: public
  /is-stream/1.1.0:
    is-stream: public
  /is-url/1.2.4:
    is-url: public
  /is-wsl/1.1.0:
    is-wsl: public
  /isarray/1.0.0:
    isarray: public
  /isexe/2.0.0:
    isexe: public
  /isurl/1.0.0:
    isurl: public
  /iterm2-version/4.2.0:
    iterm2-version: public
  /js-yaml/3.14.1:
    js-yaml: public
  /jsbn/1.1.0:
    jsbn: public
  /json-buffer/3.0.0:
    json-buffer: public
  /json-parse-better-errors/1.0.2:
    json-parse-better-errors: public
  /jsonfile/4.0.0:
    jsonfile: public
  /jwa/1.4.1:
    jwa: public
  /jws/3.2.2:
    jws: public
  /keypair/1.0.4:
    keypair: public
  /keyv/3.0.0:
    keyv: public
  /load-json-file/5.3.0:
    load-json-file: public
  /lodash._reinterpolate/3.0.0:
    lodash._reinterpolate: public
  /lodash.defaults/4.2.0:
    lodash.defaults: public
  /lodash.flatten/4.4.0:
    lodash.flatten: public
  /lodash.includes/4.3.0:
    lodash.includes: public
  /lodash.isboolean/3.0.3:
    lodash.isboolean: public
  /lodash.isinteger/4.0.4:
    lodash.isinteger: public
  /lodash.isnumber/3.0.3:
    lodash.isnumber: public
  /lodash.isplainobject/4.0.6:
    lodash.isplainobject: public
  /lodash.isstring/4.0.1:
    lodash.isstring: public
  /lodash.keyby/4.6.0:
    lodash.keyby: public
  /lodash.once/4.1.1:
    lodash.once: public
  /lodash.sortby/4.7.0:
    lodash.sortby: public
  /lodash.template/4.5.0:
    lodash.template: public
  /lodash.templatesettings/4.2.0:
    lodash.templatesettings: public
  /lodash/4.17.21:
    lodash: public
  /log-chopper/1.0.2:
    log-chopper: public
  /lowercase-keys/1.0.1:
    lowercase-keys: public
  /lru-cache/6.0.0:
    lru-cache: public
  /make-dir/3.1.0:
    make-dir: public
  /make-fetch-happen/9.1.0:
    make-fetch-happen: public
  /media-typer/0.3.0:
    media-typer: public
  /merge-descriptors/1.0.1:
    merge-descriptors: public
  /merge2/1.4.1:
    merge2: public
  /methods/1.1.2:
    methods: public
  /micromatch/4.0.5:
    micromatch: public
  /mime-db/1.52.0:
    mime-db: public
  /mime-types/2.1.35:
    mime-types: public
  /mime/1.6.0:
    mime: public
  /mimic-fn/1.2.0:
    mimic-fn: public
  /mimic-response/1.0.1:
    mimic-response: public
  /minimatch/3.1.2:
    minimatch: public
  /minimist/1.2.7:
    minimist: public
  /minipass-collect/1.0.2:
    minipass-collect: public
  /minipass-fetch/1.4.1:
    minipass-fetch: public
  /minipass-flush/1.0.5:
    minipass-flush: public
  /minipass-pipeline/1.2.4:
    minipass-pipeline: public
  /minipass-sized/1.0.3:
    minipass-sized: public
  /minipass/4.0.0:
    minipass: public
  /minizlib/2.1.2:
    minizlib: public
  /mkdirp-classic/0.5.3:
    mkdirp-classic: public
  /mkdirp/0.5.6:
    mkdirp: public
  /ms/2.1.3:
    ms: public
  /mustache/2.3.2:
    mustache: public
  /mute-stream/0.0.7:
    mute-stream: public
  /nan/2.17.0:
    nan: public
  /natural-orderby/2.0.3:
    natural-orderby: public
  /negotiator/0.6.3:
    negotiator: public
  /netrc-parser/3.1.6:
    netrc-parser: public
  /nice-try/1.0.5:
    nice-try: public
  /node-addon-api/5.0.0:
    node-addon-api: public
  /node-fetch/2.6.7:
    node-fetch: public
  /node-forge/1.3.0:
    node-forge: public
  /node-gyp/8.4.1:
    node-gyp: public
  /node-notifier/10.0.1:
    node-notifier: public
  /nopt/5.0.0:
    nopt: public
  /normalize-path/3.0.0:
    normalize-path: public
  /normalize-url/2.0.1:
    normalize-url: public
  /npm-run-path/4.0.1:
    npm-run-path: public
  /npmlog/5.0.1:
    npmlog: public
  /object-assign/4.1.1:
    object-assign: public
  /object-inspect/1.12.2:
    object-inspect: public
  /object-treeify/1.1.33:
    object-treeify: public
  /on-finished/2.4.1:
    on-finished: public
  /once/1.4.0:
    once: public
  /onetime/2.0.1:
    onetime: public
  /open/6.4.0:
    open: public
  /opn/3.0.3:
    opn: public
  /original/1.0.2:
    original: public
  /os-homedir/1.0.2:
    os-homedir: public
  /os-tmpdir/1.0.2:
    os-tmpdir: public
  /osenv/0.1.5:
    osenv: public
  /p-cancelable/0.4.1:
    p-cancelable: public
  /p-finally/1.0.0:
    p-finally: public
  /p-is-promise/1.1.0:
    p-is-promise: public
  /p-map/4.0.0:
    p-map: public
  /p-timeout/2.0.1:
    p-timeout: public
  /parse-json/4.0.0:
    parse-json: public
  /parseurl/1.3.3:
    parseurl: public
  /password-prompt/1.1.2:
    password-prompt: public
  /path-is-absolute/1.0.1:
    path-is-absolute: public
  /path-key/2.0.1:
    path-key: public
  /path-to-regexp/0.1.7:
    path-to-regexp: public
  /path-type/4.0.0:
    path-type: public
  /phoenix/1.6.15:
    phoenix: public
  /picomatch/2.3.1:
    picomatch: public
  /pify/3.0.0:
    pify: public
  /plist/3.0.6:
    plist: public
  /prepend-http/2.0.0:
    prepend-http: public
  /printf/0.6.1:
    printf: public
  /process-nextick-args/2.0.1:
    process-nextick-args: public
  /promise-inflight/1.0.1:
    promise-inflight: public
  /promise-retry/2.0.1:
    promise-retry: public
  /proxy-addr/2.0.7:
    proxy-addr: public
  /psl/1.9.0:
    psl: public
  /pstree.remy/1.1.8:
    pstree.remy: public
  /pump/3.0.0:
    pump: public
  /qs/6.11.0:
    qs: public
  /query-string/5.1.1:
    query-string: public
  /querystringify/2.2.0:
    querystringify: public
  /queue-microtask/1.2.3:
    queue-microtask: public
  /range-parser/1.2.1:
    range-parser: public
  /raw-body/2.5.1:
    raw-body: public
  /readable-stream/3.6.0:
    readable-stream: public
  /readdirp/3.6.0:
    readdirp: public
  /redeyed/2.1.1:
    redeyed: public
  /redis-errors/1.2.0:
    redis-errors: public
  /redis-parser/3.0.0:
    redis-parser: public
  /requires-port/1.0.0:
    requires-port: public
  /responselike/1.0.2:
    responselike: public
  /restore-cursor/2.0.0:
    restore-cursor: public
  /retry/0.12.0:
    retry: public
  /reusify/1.0.4:
    reusify: public
  /rimraf/3.0.2:
    rimraf: public
  /run-async/2.4.1:
    run-async: public
  /run-parallel/1.2.0:
    run-parallel: public
  /rxjs/6.6.7:
    rxjs: public
  /safe-buffer/5.2.1:
    safe-buffer: public
  /safer-buffer/2.1.2:
    safer-buffer: public
  /semver/5.6.0:
    semver: public
  /send/0.18.0:
    send: public
  /serve-static/1.15.0:
    serve-static: public
  /set-blocking/2.0.0:
    set-blocking: public
  /setprototypeof/1.2.0:
    setprototypeof: public
  /shebang-command/1.2.0:
    shebang-command: public
  /shebang-regex/1.0.0:
    shebang-regex: public
  /shell-escape/0.2.0:
    shell-escape: public
  /shell-quote/1.7.4:
    shell-quote: public
  /shellwords/0.1.1:
    shellwords: public
  /side-channel/1.0.4:
    side-channel: public
  /signal-exit/3.0.7:
    signal-exit: public
  /simple-update-notifier/1.1.0:
    simple-update-notifier: public
  /slash/3.0.0:
    slash: public
  /smart-buffer/4.2.0:
    smart-buffer: public
  /smooth-progress/1.1.0:
    smooth-progress: public
  /socks-proxy-agent/6.2.1:
    socks-proxy-agent: public
  /socks/2.7.1:
    socks: public
  /sort-keys/2.0.0:
    sort-keys: public
  /sparkline/0.2.0:
    sparkline: public
  /sprintf-js/1.0.3:
    sprintf-js: public
  /ssh2/1.11.0:
    ssh2: public
  /ssri/8.0.1:
    ssri: public
  /statuses/2.0.1:
    statuses: public
  /strftime/0.10.1:
    strftime: public
  /strict-uri-encode/1.1.0:
    strict-uri-encode: public
  /string-width/3.1.0:
    string-width: public
  /string_decoder/1.3.0:
    string_decoder: public
  /strip-ansi/5.2.0:
    strip-ansi: public
  /strip-bom/3.0.0:
    strip-bom: public
  /strip-eof/2.0.0:
    strip-eof: public
  /supports-color/5.5.0:
    supports-color: public
  /supports-hyperlinks/1.0.1:
    supports-hyperlinks: public
  /tar-fs/2.1.1:
    tar-fs: public
  /tar-stream/2.2.0:
    tar-stream: public
  /tar/6.1.13:
    tar: public
  /temp/0.9.4:
    temp: public
  /term-img/4.1.0:
    term-img: public
  /through/2.3.8:
    through: public
  /timed-out/4.0.1:
    timed-out: public
  /tmp/0.0.33:
    tmp: public
  /to-readable-stream/1.0.0:
    to-readable-stream: public
  /to-regex-range/5.0.1:
    to-regex-range: public
  /toidentifier/1.0.1:
    toidentifier: public
  /touch/3.1.0:
    touch: public
  /tr46/0.0.3:
    tr46: public
  /treeify/1.1.0:
    treeify: public
  /true-myth/2.2.3:
    true-myth: public
  /tslib/1.14.1:
    tslib: public
  /tunnel-agent/0.6.0:
    tunnel-agent: public
  /tunnel-ssh/4.1.6:
    tunnel-ssh: public
  /tweetnacl/0.14.5:
    tweetnacl: public
  /type-fest/0.21.3:
    type-fest: public
  /type-is/1.6.18:
    type-is: public
  /undefsafe/2.0.5:
    undefsafe: public
  /unique-filename/1.1.1:
    unique-filename: public
  /unique-slug/2.0.2:
    unique-slug: public
  /universalify/0.1.2:
    universalify: public
  /unpipe/1.0.0:
    unpipe: public
  /urijs/1.19.11:
    urijs: public
  /url-parse-lax/3.0.0:
    url-parse-lax: public
  /url-parse/1.5.10:
    url-parse: public
  /url-to-options/1.0.1:
    url-to-options: public
  /util-deprecate/1.0.2:
    util-deprecate: public
  /utils-merge/1.0.1:
    utils-merge: public
  /uuid/3.3.2:
    uuid: public
  /valid-url/1.0.9:
    valid-url: public
  /validator/13.7.0:
    validator: public
  /vary/1.1.2:
    vary: public
  /webidl-conversions/3.0.1:
    webidl-conversions: public
  /whatwg-url/5.0.0:
    whatwg-url: public
  /which/2.0.2:
    which: public
  /wide-align/1.1.5:
    wide-align: public
  /widest-line/2.0.1:
    widest-line: public
  /wrap-ansi/4.0.0:
    wrap-ansi: public
  /wrappy/1.0.2:
    wrappy: public
  /ws/6.2.2:
    ws: public
  /xmlbuilder/15.1.1:
    xmlbuilder: public
  /yallist/4.0.0:
    yallist: public
  /yarn/1.22.19:
    yarn: public
included:
  dependencies: true
  devDependencies: true
  optionalDependencies: true
injectedDeps: {}
layoutVersion: 5
nodeLinker: isolated
packageManager: pnpm@6.32.10
pendingBuilds: []
prunedAt: Tue, 27 Dec 2022 14:49:07 GMT
publicHoistPattern:
  - '*'
registries:
  default: https://registry.npmjs.org/
skipped:
  - /fsevents/2.3.2
storeDir: /home/workspace/.pnpm-store/v3
virtualStoreDir: .pnpm
